...
Вызывается **[glDrawArrays](#gldrawarrays)** с параметрами **`(GL_POINTS, 0, 1)`**, которая задает геометрический примитив *`GL_POINTS`* (задает точку). *`0`* задает начальный индекс в включенных массивах. *`1`* задает количество отображаемых индексов.  
