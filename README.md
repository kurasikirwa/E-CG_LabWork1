...
2.  **[gl функции](#gl-%D1%84%D1%83%D0%BD%D0%BA%D1%86%D0%B8%D0%B8)**
    ...
    * [glClear](#glclear)
    * [glEnableVertexAttribArray, glDisableVertexAttribArray (glDisableVertexArrayAttrib, glEnableVertexArrayAttrib)](#glenablevertexattribarray-gldisablevertexattribarray--gldisablevertexarrayattrib-glenablevertexarrayattrib)
    * [glBindBuffer](#glbindbuffer)
    * [glVertexAttribPointer (glVertexAttribIPointer, glVertexAttribLPointer)](#glvertexattribpointer-glvertexattribipointer-glvertexattriblpointer)
    * [glDrawArrays](#gldrawarrays)
    * [glGenBuffers](#glgenbuffers)
    * [glBufferData](#glbufferdata)
3.  **[glew функции](#glew-%D1%84%D1%83%D0%BD%D0%BA%D1%86%D0%B8%D0%B8)**
    * [GLEW (обработчик расширений OpenGL)](#glew-%D0%BE%D0%B1%D1%80%D0%B0%D0%B1%D0%BE%D1%82%D1%87%D0%B8%D0%BA-%D1%80%D0%B0%D1%81%D1%88%D0%B8%D1%80%D0%B5%D0%BD%D0%B8%D0%B9-opengl)
4.  **[Собственные функции](#%D1%81%D0%BE%D0%B1%D1%81%D1%82%D0%B2%D0%B5%D0%BD%D0%BD%D1%8B%D0%B5-%D1%84%D1%83%D0%BD%D0%BA%D1%86%D0%B8%D0%B8)**
    * [RenderSceneCB](#renderscenecb)
    * [CreateVertexBuffer](#createvertexbuffer)
5.  **[Список литературы](#%D1%81%D0%BF%D0%B8%D1%81%D0%BE%D0%BA-%D0%BB%D0%B8%D1%82%D0%B5%D1%80%D0%B0%D1%82%D1%83%D1%80%D1%8B)**
...
## GLEW (обработчик расширений OpenGL)
Библиотека [OpenGL Extension Wrangler](http://glew.sourceforge.net/) обеспечивает доступ ко всем точкам входа GL. Он поддерживает Windows, MacOS X, Linux и FreeBSD.  

Как и в случае с большинством других загрузчиков, вы не должны включать gl.h , glext.h или любой другой заголовочный файл, связанный с gl, перед glew.h , иначе вы получите сообщение об ошибке, что вы включили gl.h перед glew.h. На самом деле вам вообще не следует включать gl.h ; glew.h заменяет его.  

GLEW также предоставляет wglew.h , который предоставляет специфичные для Windows функции GL (функции wgl). Если вы включите wglext.h перед wglew.h , GLEW будет жаловаться. GLEW также предоставляет glxew.h для систем X Windows. Если вы включите glxext.h перед glxew.h , GLEW будет жаловаться.  

Последний выпуск по состоянию на июнь 2021 года — версия 2.2.0.

### Инициализация GLEW 1.13.0 и более ранних версий
**GLEW** до версии 1.13.0 имеет проблемы с основными контекстами. Он вызывает **glGetString(GL_EXTENSIONS)** , что вызывает *GL_INVALID_ENUM* в основном контексте **GL 3.2+**, как только вызывается **`glewInit()`**. Он также не получает указатели функций. **GLEW* версии 2.0.0+** вместо этого использует **glGetStringi**. Единственное исправление для более ранних версий — использовать **glewExperimental**:  

```c
// При использовании GLEW версии 1.13 или более ранней 
glewExperimental=TRUE;
  GLenum err=glewInit();
  if(err!=GLEW_OK) {
    // Проблема: ошибка glewInit, что-то серьезно не так.
    cout << "glewInit failed: " << glewGetErrorString(err) << endl;
    exit(1);
  }
```

**glewExperimental** — это переменная, которая уже определена в **GLEW**. Вы должны установить его в *GL_TRUE* перед вызовом **`glewInit()`**.  

Вы все равно можете получить *GL_INVALID_ENUM* (в зависимости от используемой вами версии **GLEW**), но, по крайней мере, **GLEW** игнорирует **glGetString(GL_EXTENSIONS)** и получает все указатели функций.  

Если вы создаете контекст **GL** по-старому или создаете обратно совместимый контекст для **GL 3.2+**, вам не нужен **glewExperimental**.  
***
...
Вызывается **[glVertexAttribPointer](#glvertexattribpointer-glvertexattribipointer-glvertexattriblpointer)** с параметрами **`(0, 3, GL_FLOAT, GL_FALSE, 0, 0)`**, который указывает расположение и формат данных массива универсальных атрибутов вершины в индексе *`0`*, которые будут использоваться при отрисовке. *`3`* указывает количество компонентов. *`GL_FLOAT`* задает тип данных каждого компонента. *`GL_FALSE`* преобразовывает значение в плавающие числа непосредственно без нормализации. *`0`* задает шаг байта от одного атрибута к другому. *`0`* обрабатывается как смещение байта в хранилище данных объекта буфера.  

Вызывается **[glDrawArrays](#gldrawarrays)** с параметрами **`(GL_TRIANGLES, 0, 1)`**, которая задает геометрический примитив *`GL_TRIANGLES`* (задает треугольник). *`0`* задает начальный индекс в включенных массивах. *`3`* задает количество отображаемых индексов.  
...
## CreateVertexBuffer
CreateVertexBuffer - создает буфер Vertex.
### С спецификация
```c
...
### Описание
Инициализируется массив вершин *Vertices* в который записываются координаты вершин.  

Вызывается **[glGenBuffers](#glgenbuffers)** с параметрами **`(1, &VBO)`**, который генерирует *1* имя буфера *VBO*.  

Вызывается **[glBindBuffer](#glbindbuffer)** с параметрами **`(GL_ARRAY_BUFFER, VBO)`** который устанавливает *GL_ARRAY_BUFFER* (Атрибуты вершин) и *VBO* привязывает это имя объекта буфера к целевому объекту.  

Вызывается **[glBufferData](#glbufferdata)** с параметрами **`(GL_ARRAY_BUFFER, sizeof(Vertices), Vertices, GL_STATIC_DRAW)`**, который создает новое хранилище данных для объекта буфера, который в настоящее время привязан к *GL_ARRAY_BUFFER*. Новое хранилище данных создается с размером массива *Vertices* в байтах и данными из массива *Vertices*. *GL_STATIC_DRAW* задает шаблон использования хранилища данных, а именно отрисовывание данных.  
